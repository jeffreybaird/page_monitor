...
export const desktopNotifier: Notifier = {
  ...
  async send({ monitor, change }) {
    if (!(await notificationsAllowed()))
      throw new Error(
        `Desktop notifications are disabled. Allow them in ${browserName}/system settings.`,
      );
    await api().notifications.create(`change:${monitor.id}:${change.id}`, {
      type: 'basic',
      ...
      ...notificationContext(new URL(monitor.url).hostname),
    });
  },
};
...
export async function updateBadge(monitors: Monitor[]): Promise<void> {
  ...
  await api().action.setBadgeBackgroundColor({ color: '#185b46' });
}
...
export async function notifyRendering(id: string, name: string): Promise<void> {
  ...
  await api().notifications.create(`rendering:${id}`, {
    type: 'basic',
    iconUrl: api().runtime.getURL('icons/128.png'),
